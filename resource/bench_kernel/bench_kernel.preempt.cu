...
__device__  void loop_device(dim3 task_idx, dim3 thread_idx, float* __restrict__ n, float* __restrict__ b, float* __restrict__ temp){
...
extern "C" __global__ void loop(volatile int* preempted, int task_num, int task_offset, int* task_slot, float** args) {
    ...
    while (true) {
        ...
        loop_device(task_idx, thread_idx, (float* __restrict__)args[0], (float* __restrict__)args[1], (float* __restrict__)args[2]);
    }
}
