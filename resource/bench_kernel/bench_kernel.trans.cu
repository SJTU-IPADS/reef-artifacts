...
extern "C" __global__  __attribute__((amdgpu_num_vgpr(25))) __attribute__((amdgpu_num_sgpr(30))) void loop_device_wrapper(float* __restrict__ n, float* __restrict__ b, float* __restrict__ temp) {
    ...
    loop_device((float* __restrict__)n,(float* __restrict__)b,(float* __restrict__)temp);
    asm volatile(";; end_flag"); // jump back to the caller
}

extern "C" __global__  void loop(
